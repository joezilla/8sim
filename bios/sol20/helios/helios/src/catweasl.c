 ...
 * $Id: catweasl.c,v 1.15 2003/06/19 00:04:39 mann Exp $
 *
...
#define STEP_USEC 8000 /* 8 ms per track stepped  (just to be safe) */
...
CWTriggerStep(catweasel_contr *c)
{
    ...
    catweasel_usleep(STEP_USEC-1000);
}
...
catweasel_detect_drive(catweasel_drive *d)
{
    ...
    printf("Before stepping, track 0 sensor is ");
    if (CWTrack0(c))
	printf("on\n");
    else
	printf("off\n");
	...
	catweasel_usleep(100);  // dir must be stable before step
	for (i=0; i<3; i++) {
	    CWTriggerStep(c);
	}
	if (CWTrack0(c)) {
    ...
    catweasel_usleep(100);  // dir must be stable before step
    for (j=0; j<90 && !CWTrack0(c); j++) {
	CWTriggerStep(c);
    }
	
    if (j < 90) {
	/* drive with working track 0 sensor */
	d->type = 1;
	d->track = 0;
	printf("Had to step %d times to get to track 0\n", j);
    } else {
	/* drive without working track 0 sensor, or no drive */
	d->type = 0;
	printf("Track 0 not seen even after 90 steps\n");
    }
...
/* 1=14 MHz, 2=28 MHz, 4=56.644 MHz */
static int
...
int
catweasel_read(catweasel_drive *d, int side, int clockmult, int time)
{
    catweasel_contr *c = d->contr;
    ...
    OUTREG(c, CatOption, 0);     /* store index bit as msb */
    ...
    /* add data end mark -- catweasel doesn't indicate how much was saved */
