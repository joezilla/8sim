...
;     RELEASE   3/27/77
;
; [ BOOTLOAD ROM 'BOOT' COMMAND DISASSEMBLED, COMMENTED,
;   AND ADDED TO SOLOS.ASM SOURCE, CREATING THIS FILE,
;   BY JIM BATTLE, 5/20/2002 ]
;
;
;
;
...
;	***** BOOT COMMAND *****
;
;	READ THE BOOT SECTOR OFF THE HELIOS DRIVE AND JUMP THERE.
;
;
; F7 OUTPUT PORT ASSIGNMENT:
;    0 : 0=STEP
;    1 : 0=STEP TO HIGHER TRACK, 1=STEP TO LOWER TRACK
;    2 : 0=SELECT UNIT 2,3,6,or 7; 1=SELECT UNIT 0,1,4,or 5
;    3 : 0=SELECT UNIT 4,5,6,or 7; 1=SELECT UNIT 0,1,2,or 3
;    4 : 0=RESTORE=SEEK TRACK 0
;    5 : 0=LOAD HEAD 0 (OF UNIT 0,2,4,or 6)
;    6 : 0=LOAD HEAD 1 (OF UNIT 1,3,5,or 7)
;    7 : 0=SELECT UNIT 1,3,5,or 7; 1=SELECT UNIT 0,2,4,or 6
; UNIT SELECTED = {~b3,~b2,~b7}
;
BOOT:
	MVI	A,0CFH	; SELECT UNIT 0; SEEK TRACK 0
	OUT	0F7H	; COMMAND OUTPUT PORT
	OUT	0F5H	; RESET CONTROLLER FLAGS (LOW TRANSFER ADDRESS PORT)
	MVI	A,0FFH	; CANCEL ANY PREVIOUS COMMAND
	OUT	0F1H	; TRANSFER COMMAND PORT
SEEKWT:	IN	0F0H	; STATUS INPUT PORT
	ANI	040H	; SEEK COMPLETE FLAG
	JNZ	SEEKWT	; WAIT UNTIL SEEK IS DONE
;
	MVI	A,0DFH	; UNIT 0; LOAD HEAD 0
	OUT	0F7H	; COMMAND OUTPUT PORT
W4IDX:	IN	0F0H	; STATUS INPUT PORT
	RLC		; TEST BIT 7, INDEX MARK
	JC	W4IDX	; LOOP UNTIL INDEX SEEN
;
	LXI	B,01290H  ; =4752 LOOPS = ~55.7 MS @ 2.04 MHZ
WAIT56:
	DCX	B	; 5
	MOV	A,B	; 5
	ORA	C	; 4
	JNZ	WAIT56	; 10
;
W4IDX2:	IN	0F0H	; STATUS INPUT PORT
	RLC		; TEST BIT 7, INDEX MARK
	JC	W4IDX2	; LOOP UNTIL INDEX SEEN
;
W4RDY:	IN	0F0H	; STATUS INPUT PORT
	ANI	02H	; CONTROLLER READY
	JZ	W4RDY	; WAIT UNTIL CONTROLLER READY
;
	XRA	A
	OUT	0F5H	; LOW TRANSFER ADDRESS PORT
	OUT	0F6H	; HIGH TRANSFER ADDRESS PORT
	LXI	H,0340H  ; NUMBER OF BYTES TO READ = 832 BYTES
	MOV	A,L
	OUT	0F3H	; LOW BYTE COUNT PORT
	MOV	A,H
	OUT	0F4H	; HIGH BYTE COUNT PORT
	MVI	A,03H	; READ DATA COMMAND
	OUT	0F1H	; TRANSFER COMMAND PORT
RDBOOT:
	IN	0F0H	; STATUS INPUT PORT
	ANI	01BH	; CRC CHECK COMPLETE | CRC ERROR FLAG | CONTROLLER READY | TRANSFER COMPLETE
	JZ	RDBOOT	; WAIT FOR *SOMETHING* TO HAPPEN
	ANI	08H	; CRC ERROR FLAG
	JNZ	BOOT	; TRY AGAIN (...AND AGAIN...)
	IN	0F0H	; STATUS INPUT PORT
	ANI	03H	; TRANSFER COMPLETE | CONTROLLER READY
	JZ	RDBOOT
	RST	0	; JUMP TO BOOT SECTOR MEMORY IMAGE
	HLT		; SHOULDN'T EVER BE EXECUTED
;
;
;
;	***** DUMP COMMAND *****
...
DLOP1:	DCX	D
	...
	DB	00H	; pad out to exactly fill 2KB
;
;
;********* END OF PROGRAM ************
;
